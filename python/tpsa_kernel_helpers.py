...
T_END            = 3e-9 # 1.2e-8          # end of integration                [s]
...
# ---------------------------------------------------------------------
PERTURB_ORDER = 6                   # maximum perturbation order K       [1]
NUM_STEPS     = 300                 # number of fixed time steps         [1]
...
@njit(cache=True, fastmath=True)
def _kin0_source_high(t, r0, p0, r_out, v_out, a_out, j_out, s_out):
    dt_probe = 1e-12
    if t > 0.0 and 0.25 * t < dt_probe:
        dt_probe = 0.25 * t
    if dt_probe < 1e-15:
        dt_probe = 1e-15
    ...
    _kin0_source(t_m, r0, p0, r_m, v_m, a_m, j_m)
    _kin0_source(t,   r0, p0, r_out, v_out, a_out, j_out)
    _kin0_source(t_p, r0, p0, r_p, v_p, a_p, j_p)

    inv_2dt = 0.5 / dt_probe
    s_out[0] = (j_p[0] - j_m[0]) * inv_2dt
    s_out[1] = (j_p[1] - j_m[1]) * inv_2dt
    s_out[2] = (j_p[2] - j_m[2]) * inv_2dt
...
def _eval_derivs(t_obs, Y_r, Y_p, hist_r, hist_p, n_hist, dt,
    ...
    for i in prange(N):
        ...
        j0_an  = np.empty(3);        s0_an  = np.empty(3)
        r_j    = np.zeros((K1, 3));  v_j    = np.zeros((K1, 3))
        p_j    = np.zeros((K1, 3));  bdot_j = np.zeros((K1, 3))
        delta  = np.zeros(K1);       d2_buf = np.zeros(K1);  d3_buf = np.zeros(K1)
        ...
        if t_obs > 1e-18 and K >= 1:
            for j in range(N):
                ...
                g0   = _kin0_source_high(t_r0, r0_parts[j], p0_parts[j],
                                         r0_an, v0_an, a0_an, j0_an, s0_an)
                ...
                # higher TPSA orders: interpolated source history plus a local
                # retarded-time Taylor expansion through the cubic term.
                for k in range(1, K1):
                    for c in range(3):
                        r_j[k, c]    = 0.0
                        v_j[k, c]    = 0.0
                ...
                # source position in delta: r_j(t_r0) + v_j(t_r0) * delta
                # + 1/2 a_j(t_r0) * delta^2 + 1/6 j_j(t_r0) * delta^3.
                ...
                # We compose with analytic 0-th order time derivatives through
                # the cubic term so the TPSA ring can populate K >= 3 orders.
                tp_mul(delta, delta, d2_buf)
                tp_mul(d2_buf, delta, d3_buf)
                for k in range(1, K1):
                    r_j[k, 0] += v0_an[0] * delta[k] + 0.5 * a0_an[0] * d2_buf[k] + (1.0 / 6.0) * ad0_an[0] * d3_buf[k]
                    r_j[k, 1] += v0_an[1] * delta[k] + 0.5 * a0_an[1] * d2_buf[k] + (1.0 / 6.0) * ad0_an[1] * d3_buf[k]
                    r_j[k, 2] += v0_an[2] * delta[k] + 0.5 * a0_an[2] * d2_buf[k] + (1.0 / 6.0) * ad0_an[2] * d3_buf[k]
                    v_j[k, 0] += a0_an[0] * delta[k] + 0.5 * ad0_an[0] * d2_buf[k] + (1.0 / 6.0) * s0_an[0] * d3_buf[k]
                    v_j[k, 1] += a0_an[1] * delta[k] + 0.5 * ad0_an[1] * d2_buf[k] + (1.0 / 6.0) * s0_an[1] * d3_buf[k]
                    v_j[k, 2] += a0_an[2] * delta[k] + 0.5 * ad0_an[2] * d2_buf[k] + (1.0 / 6.0) * s0_an[2] * d3_buf[k]
                    bdot_j[k, 0] += (ad0_an[0] * delta[k] + 0.5 * s0_an[0] * d2_buf[k]) * inv_C
                    bdot_j[k, 1] += (ad0_an[1] * delta[k] + 0.5 * s0_an[1] * d2_buf[k]) * inv_C
                    bdot_j[k, 2] += (ad0_an[2] * delta[k] + 0.5 * s0_an[2] * d2_buf[k]) * inv_C
...
# Plotting configuration: number of lower orders drawn underneath the
# top order.  Set to 0 to plot only the K-th order curve (in which case
# the reference-data difference subplot is shown as before).
N_LOWER_ORDERS_TO_PLOT = K
# In[11]:
...
_lw        = lambda k: 1.5 if k == K else 1.
_col       = lambda k: 'orange' if k == K else None
